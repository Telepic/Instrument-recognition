...
class Model(object):
    ...
    def sample_records(self, rec_id, count=-1, fixed_stride=-1, pitch_shift=0):
        mse = 0.
        mse_breakdown = dict()
        avp_breakdown = dict()
        ...
        if count == -1:
            if self.isTest:
                count = int((len(rec_id) / 4) / 512)
            else:
                count = int(((len(self.data[rec_id][0]) / 4)) / 512)
        # print(count)
        offset = 0  # 44100
        ...
        if self.isTest:
            stride = (len(rec_id) / 4 - offset - int(
                sf * self.window)) / count if fixed_stride == -1 else fixed_stride
        else:
            stride = (self.data[rec_id][1] - offset - int(
                sf * self.window)) / count if fixed_stride == -1 else fixed_stride
        ...
        for j in range(count):
            ...
            X[j, 0, :, 0], Y[j] = self.get_data(rec_id, offset + j * stride, pitch_shift=int(round(pitch_shift)),
                                                scaling_factor=sf)
